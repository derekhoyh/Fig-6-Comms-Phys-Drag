...
nrms1=2; %nrms of active layer in units of 10^10 cm^-2
nrms2=2;  %nrms of passive layer in units of 10^10 cm^-2
eta=-0.5; %correlation coefficient in the bivariate normal distribution
ng1=(40:0.5:60); %avrg density of active layer in units of 10^10 cm^-2.
...
dragcond=@(n1,n2) -5.*min(abs(n1),abs(n2)).*(1-sign(n1).*sign(n2))./2+ 1e-3.*sqrt(abs(n1).*abs(n2)).*(1+sign(n1).*sign(n2))./2;
...
save(['data-nrms1-' num2str(nrms1) '.mat'])
